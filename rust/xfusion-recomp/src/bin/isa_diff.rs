...
fn main() {
    ...
    for cur in lines {
        ...
        // Structural: only compare flag bits the .isa DECLARES this def writes
        // (DEF_FLAGS_MASK, generated by RustLiftGen from every `(= FLAG ...)` in
        // the template). SDM-undefined flags (ROR-OF count>1, IMUL SF/ZF, logical-AF)
        // are then filtered by construction — the .isa doesn't write them, so we don't
        // compare them. Stronger than a global mask: per-def, exactly what's declared.
        // ISA_DIFF_FLAGMASK stays as an outer intersection (default 0x8C1 = the four).
        // Env ISA_DIFF_NO_DEFMASK=1 disables this (for hunting SDM-undefined behavior).
        let ins_flag_mask = if std::env::var("ISA_DIFF_NO_DEFMASK").is_ok() { 0xFFF }
            else { *DEF_FLAGS_MASK.get(d.def_id as usize).unwrap_or(&0) };
        ...
        let fm = eflags_mask & ins_flag_mask;
        if !skip_shim && (st.eflags & fm) != (cur.eflags & fm) {
            flag_diverge_bits = (st.eflags ^ cur.eflags) & fm;
            if !diverged {
                eprintln!("\n=== DIVERGE @seq {} rva {:#x} [{}] {:02x?} ===",
                    prev.seq, pc_rva, mnem, &bytes[..d.len as usize]);
            }
