...
//
// `IlUnimpl` is the lifter self-‡ (spec §4): the lifter says "I don't model this"
...
public enum RegKind { X, W, Sp, Wsp, V, Nzcv, Pc, Sys,
    // Maxwell shader regs (tier-3 M1, day-28). Gpr = R0-R254 (32-bit
...
public abstract record Il(IlType Ty) {
    ...
    static string Pp(Il il, int d) => il switch {
        ...
        IlVecBuild(var et, var es) => $"(v128 build.{et} {string.Join(" ", es.Select(e => Pp(e, d)))})",
        IlVecAll(var et, var s) => $"(v128 all.{et} {Pp(s, d)})",
        IlVecElem(var t, var v, var i) => $"({t} elem {Pp(v, d)} {Pp(i, d)})",
        IlVecZeroTop(var v) => $"(v128 zerotop {Pp(v, d)})",
        IlVecBin(var et, var op, var l, var r) => $"(v128 v{op.ToString().ToLower()}.{et} {Pp(l, d)} {Pp(r, d)})",
        IlVecUn(var et, var op, var x) => $"(v128 v{op.ToString().ToLower()}.{et} {Pp(x, d)})",
        ...
        // (T6)×6 kt[2]-at-printer: (?IlIntrin) read as
...
// IlVecElem = extract scalar lane; IlVecZeroTop = zero high 64.
public record IlVecBuild(IlType ElemTy, IReadOnlyList<Il> Elems) : Il(new IlType.Vec(128));
public record IlVecAll(IlType ElemTy, Il Scalar) : Il(new IlType.Vec(128));
public record IlVecElem(IlType Ty, Il Vec, Il Idx) : Il(Ty);
public record IlVecZeroTop(Il Vec) : Il(new IlType.Vec(128));
public record IlVecBin(IlType ElemTy, BinOp Op, Il L, Il R) : Il(new IlType.Vec(128));
public record IlVecUn(IlType ElemTy, UnOp Op, Il X) : Il(new IlType.Vec(128));
