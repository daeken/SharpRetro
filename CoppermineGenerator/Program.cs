...
public class Program : Core {
	public static void Main(string[] args) {
		var defs = Core.ParseSpec(File.ReadAllText("coppermine.isa"), new(), CoppermineDef.Parse).Select(x => (CoppermineDef) x).ToList();
		BuildDisassembler(defs);
		//BuildInterpreter(defs);
		//BuildRecompiler(defs);
		
		//CleanupCode("../CoppermineCpu/Generated");
	}
	
	static void BuildDisassembler(List<CoppermineDef> defs) {
		...
		using var fp = File.Open("../CoppermineCpu/Generated/Disassembler.cs", FileMode.Create);
	...
	static void GenerateFields(CodeBuilder c, CoppermineDef def) {
