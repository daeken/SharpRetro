...
// Proof-of-form: 4 heads. The runtime-emit shape (rt) = `b.method(lhs, rhs)`;
...
public static class ScalarMathEmit {
    public static void Register() {
        ...
        // rt = `b.add(Lift(l), Lift(r))` — Lift() ensures both args are runtime Vals.
        Expression(new[] { "+" },
            list => $"({GenerateExpression(list[1])}).wrapping_add({GenerateExpression(list[2])})",
            list => $"b.add({Lift(list[1])}, {Lift(list[2])})");
        Expression(new[] { "-" },
            list => $"({GenerateExpression(list[1])}).wrapping_sub({GenerateExpression(list[2])})",
            list => $"b.sub({Lift(list[1])}, {Lift(list[2])})");
        Expression(new[] { "&" },
            list => $"(({GenerateExpression(list[1])}) & ({GenerateExpression(list[2])}))",
            list => $"b.and({Lift(list[1])}, {Lift(list[2])})");
        Expression(new[] { "==" },
            list => $"(({GenerateExpression(list[1])}) == ({GenerateExpression(list[2])}))",
            list => $"b.eq({Lift(list[1])}, {Lift(list[2])})");
    }
}
